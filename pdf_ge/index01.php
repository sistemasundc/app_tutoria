...
// -----------TUTOR DE AULA -------ASISTENCIA Y FORMATO--------------------
        function DatosFormatoAsistenci($id_sesion,  $semestre) {
            $sql = "SELECT
                    ...
                    d.email_doce,  -- este será el índice [5]
...
// -----------TUTOR DE CURSO -------ASISTENCIA Y FORMATO--------------------
        function DatosFormatoAsistenciaCurso($id_sesion, $semestre) {
            $sql = "SELECT
                        ...
                        s.tema,
                        s.compromiso_indi,
                        2 AS idtipo  -- Siempre es grupal
                    FROM tutoria_sesiones_tutoria_f78 s
                    INNER JOIN carga_lectiva cl ON s.id_cargalectiva = cl.id_cargalectiva
        ...
        public function EstudiantesAsistencia_TC($id_sesion) {
            $sql = "SELECT 
                        CONCAT(e.apepa_estu, ' ', e.apema_estu, ' ', e.nom_estu) AS estu
                    FROM tutoria_detalle_sesion_curso d
                    INNER JOIN estudiante e ON d.id_estu = e.id_estu
                    WHERE d.sesiones_tutoria_id = $id_sesion
                    AND d.marcar_asis_estu = 0";
