   ...
   $.ajax({
        ...
        if (data.length>0) {
          ...
          $.each(data, function(i,elemt) {
            html += "<div class='box'>";
              html += "<div class='box-header with-border' onclick='toggleDiv("+elemt.idfolder+")' style='cursor: pointer;'>";
                html += "<i class='fa fa-folder-open fa-4x' style='color:#248ae1'></i>";
                html += "<h3 class='box-title'>"+elemt.namefolder+"</h3>";
              html += "</div>";
              html += "<div class='box-body contenido_foldes' id='contenfiles"+elemt.idfolder+"'>";
              html += "</div>";
              html += "</div>";
            html += "</div>";
...
function ListarFilesFolderSelecionado(id) {
  ...
  $.ajax({
        url: '../controlador/folders/ControllerGetFiles_docente.php',
        type: 'POST',
...
function Recorer_Files_Folder(data, idfolder) {
 ...
 // html += "<span><i class='fa fa-upload'></i> Seleccionar archivos</span>";
  ...
  html += "</label>";
  html += "&nbsp;<button class='btn btn-personalizado btn-sm' id='buttum_register" + idfolder + "\
  ' onclick='GuardarFiles(" + idfolder + ")' style='display: inline-block;'><i class='fa fa-save'></i> Guardar</button>"; // Agregar estilo inline
...
function GuardarFiles(idforder){
    ...
    $.ajax({
			...
			if (data.length > 0) {
				...
				var selectHTML = "<select id='idgrados_asig' class='js-example-basic-single' style='width:100%;'>\
          <option value='1'>Todos</option>\
          <option value='2'>Grupal</option>";
				for (var i = 0; i < data.length; i++) {
					selectHTML += "<option value='" + data[i]['id_estu'] + "'>" + data[i]['nombre'] + "</option>";
				}
				selectHTML += "</select>";
				...
				Swal.fire({
					...
					if (result.value) {
						var alumnos_asignados_tuto = $('#idgrados_asig').val();

						formData.append('alumno_ver', alumnos_asignados_tuto);
						formData.append('idfolder', idforder);
        ...
        return Swal.fire("Mensaje de advertencia", "No se encontraron alumnos asignados.", "warning");
			}
		})
