...
function ListarGradosDocente() {
   ...
   $.ajax({
        ...
        var cadena = "";
        if (data.length > 0) {
             cadena += "<option value='" + identi+ "' selected disabled>" + nameCombo + "</option>";
            for (var i = 0; i < data.length; i++) {
                cadena += "<option value='" + data[i]['id_asig'] + "'>" + data[i]['ciclos'] + "</option>";
            }
...
function enviarArchivos() {
  ...
  console.log(gradosId);
  if (gradosId == null) {
     return Swal.fire("Mensaje de advertencia", "Seleccione un ciclo", "warning");
  }
  var fileInput = document.getElementById('fileInput');
  if (fileInput.files.length == 0) {
     return Swal.fire("Mensaje de advertencia", "Seleccione archivos", "warning");
  }
...
function verArchivo(nombreArchivo, extension) {
  ...
  if (extensionMinuscula === 'mp4' || extensionMinuscula === 'pdf' || extensionMinuscula === 'docx' || extensionMinuscula === 'pptx'||extensionMinuscula === 'text') {
    // Abrir el archivo PDF en una nueva pestaña del navegador
