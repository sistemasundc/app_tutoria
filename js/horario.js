...
function listar_combo_niveles() {
    $.ajax({
        ...
        if (data.length > 0) {
            ...
            $('#tipo_asig').html(cadena);////lamndo en crer horari
    ...
    // var idsemet=$("inpHsemestreH").val();
                ...
                document.addEventListener("drop", function (event) {
                    ...
                    if (event.target.className == "dropzone") {
                        ...
                        //var id_tipo = event.target.getAttribute("idtipo");
                        ...
                        guardarhorario(hora, dia, curso, idtd, idtipo);
...
function guardarhorario(hora, dia, idcurso, idtd, id_tipo) { 
...
function Listar_alumnos_asignados() {
    var tipo_asignacion = $('#tipo_asig').val();
    var id_docente = $('#textId').val();

    if (tipo_asignacion == '1'){
        Listar_alumnos_individuales(tipo_asignacion, id_docente);
    }else {
        Listar_alumnos_grupales(tipo_asignacion, id_docente);
    }
}
function Eliminar_alumno_horario(idhorario, idtipo, hora, dia) {
    $.ajax({
    "url": "../controlador/docente/controlador_quitar_horario_alumno.php",
        type: 'POST',
        data: {
        ...
        if (resp > 0) {
            Swal.fire("Mensaje De Confirmacion", "Datos correctamente, Eliminado", "success").then((value) => {
                cargar_contenido('contenido_principal','horario/vista_crear_horario.php');  
            });
               
        } else {
            Swal.fire("Mensaje De Advertencia", "No se pudo completar esta accion.", "warning");
        }
    })
}
function Listar_alumnos_grupales(tipo_asignacion, id_docente){
    $.ajax({
        "url": "../controlador/docente/controlador_alumno_asignados.php",
        type: 'POST',
        data: {
            id_doce: id_docente,
        ...
        if (data.length > 0) {
            cadena += "<div class='external-event  ui-draggable ui-draggable-handle bg-primary' idcurso='\
                "+ data[0][1] + "'draggable='true' ondragstart='event.dataTransfer.setData(\'text/plain\',null)' \
                style='margin-bottom: 5px;padding: 5px;border-radius:4px;position: relative;'>Grupo de alumnos</div>";

            for (var i = 0; i < data.length; i++) {
                cadena += "<div style='margin-bottom: 5px;padding: 5px;border-radius:4px;'>" + data[i][2] + "</div>";
            }

            $('#alumnos_asignados').html(cadena);

        } else {
            cadena += "<h4>NO SE ENCONTRARON REGISTROS</h4>";
            $("#alumnos_asignados").html(cadena);
...
function Listar_alumnos_individuales(tipo_asignacion, id_docente){
    $.ajax({
        "url": "../controlador/docente/controlador_alumno_asignados.php",
        type: 'POST',
        data: {
        ...
        if (data.length > 0) {
            for (var i = 0; i < data.length; i++) {
                cadena += "<div class='external-event  ui-draggable ui-draggable-handle bg-primary' idcurso='\
                "+ data[i][1] + "'draggable='true' ondragstart='event.dataTransfer.setData(\'text/plain\',null)' \
                style='margin-bottom: 5px;padding: 5px;border-radius:4px;position: relative;'>" + data[i][2] + "</div>";
            }
    ...
    var horarios= JSON.stringify(selecionado);
      ...
      $.ajax({
        ...
        data: {
            ...
            if(resp==100){
              return Swal.fire("Mensaje De Advertencia", "Horario ya existe.", "warning");
            }

            if(resp==500){
              return Swal.fire("Mensaje De Advertencia", "Debes ingresar al menos un campo.", "warning");
            } 
         });
