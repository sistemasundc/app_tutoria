...
function ListarFilesFolderSelecionado(id) {
  ...
  var idfolder=id;
   $.ajax({
        url: '../controlador/folders/ControllerGetFiles.php', //--- alumno
        type: 'POST',
...
function RecorerFiles(data,idfolder){
 ...
 if (data.length > 0) {
   $("#fileCount"+idfolder).html(data.length);
        ...
        html += "</div>";
       $("#contenfiles"+idfolder).html(html);
           
        } else {

        $("#contenfiles"+idfolder).html('<h6 class="text"> No hay Archivos. </h6>');
