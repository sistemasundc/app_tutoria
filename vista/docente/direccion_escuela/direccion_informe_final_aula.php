...
$params = [$id_director, $semestre, $id_director, $semestre, $semestre, $id_car_director];
$types = "iiiiii";
...
<div class="container">
  ...
  <a style="float: right; margin-top: -15px;" href="direccion_escuela/reporte_envios_informe_final.php" 
      ...
      <?php if (count($filas) > 0): ?>
        <?php foreach ($filas as $row): ?>
          ...
          <tr>
            ...
            <td>
              <?= !empty($row['fecha_presentacion']) ? date('d/m/Y', strtotime($row['fecha_presentacion'])) : '<span class="text-muted">--</span>' ?>
            </td>
            <td>
              <?php if ($row['estado_envio'] == 2): ?>
                <?php
                  $mesInforme = strtolower(date('F', strtotime($row['fecha_presentacion'])));
                  $mesMap = ['april' => '04', 'may' => '05', 'june' => '06', 'july' => '07'];
                  $mesFinal = $mesMap[$mesInforme] ?? '05';
                ?>
                 <a href="#" onclick="abrirVentanaPopup('tutor_aula/vista_prev_informe_final.php?id_cargalectiva=<?= $row['id_cargalectiva'] ?>&id_docente=<?= $row['id_doce'] ?>'); return false;" class="btn btn-primary">Ver Informe</a>
              <?php else: ?>
            ...
            <td>
              <?php if ($row['estado_envio'] == 2 && !$row['estado_revision']): ?>
                <form method="POST" action="direccion_escuela/direccion_guardar_revision_informe_final.php" onsubmit="return confirm('¿Confirmar esta acción?')">
