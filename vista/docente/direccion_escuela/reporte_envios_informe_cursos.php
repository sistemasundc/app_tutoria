...
SELECT DISTINCT
    ...
    d.email_doce
FROM carga_lectiva cl
JOIN docente d ON cl.id_doce = d.id_doce
JOIN asignatura asi ON asi.id_asi = cl.id_asi
WHERE cl.id_car = ? AND cl.id_semestre = ? AND cl.tipo='M'
";
...
<head>
  ...
  <style>
    ...
    .fila-verde { background: #d4efdf; font-size: 12px; }
    .fila-roja { background: #f5b7b1; font-size: 12px;}
    .fila-amarilla { background:rgb(238, 218, 153); font-size: 12px; }
    .filtro-form select { margin-right: 20px; padding: 5px; }
    .boton-imprimir { float: right; background: #1a5276; color: white; padding: 8px 15px; border: none; cursor: pointer; }
...
<div class="contenedor">
  ...
  <form method="GET" class="filtro-form">
    <center><label><strong>Ciclo:</strong>
    ...
    <label><strong>Mes:</strong>
      <select name="mes" onchange="this.form.submit()">
        ...
        <?php foreach (["agosto","septiembre", "octubre", "noviembre", "diciembre"] as $mes): ?>
  ...
  <table>
    ...
    <?php if (empty($enviaron)): ?>
      <tr><td colspan="8" style="font-size: 12px;">Ningún docente ha enviado.</td></tr>
  ...
  <table>
    ...
    <?php if (empty($no_enviaron)): ?>
      <tr><td colspan="7" style="font-size: 12px;">Todos los docentes cumplieron con el envío del informe.</td></tr>
   <?php else: foreach ($no_enviaron as $row): ?>
    <?php
      $clase = 'fila-roja';
      if ($row['estado'] === 'GUARDADO (sin envío)') {
          $clase = 'fila-amarilla';
      }
    ?>
    <tr class="<?= $clase ?>">
      ...
      <td><?= $row['correo'] ?></td>
      <td>
        <?= ($row['estado'] === 'CUMPLIÓ' && $clase === 'fila-amarilla') ? 'GUARDADO (sin envío)' : $row['estado'] ?>
      </td>
    </tr>
  <?php endforeach; endif; ?>
    </table>
