...
if (empty($id_car) || empty($id_semestre)) {
...
// Obtener tutores asignados por carrera y semestre
$sql_tutores = "
SELECT 
    d.id_doce,
    d.abreviatura_doce,
...
$sql_estado = "
SELECT id_cargalectiva, estado_envio, fecha_presentacion 
...
foreach ($tutores as $doc) {
    $id_carga = $doc['id_cargalectiva'];
    $estado_info = $estado_docente[$id_carga]['estado'] ?? null;
    $fecha_envio = $estado_docente[$id_carga]['fecha'] ?? null;

    if ($estado_info === 2) {
        $doc['estado_final'] = 'CUMPLIÓ';
        $doc['fecha_envio'] = $fecha_envio;
        $enviaron[] = $doc;
    } elseif ($estado_info === 1) {
...
<head>
    <meta charset="UTF-8">
    <title>Relación de Docentes Tutores - Informe Final</title>
    <style>
...
<body>
    ...
    <div class="contenedor">
        <h2>RELACIÓN DE INFORMES FINALES - SEMESTRE <?= htmlspecialchars($nombre_semestre) ?></h2>
        <h2 style="font-weight: normal;">ESCUELA PROFESIONAL DE <?= $nombre_carrera ?></h2>

        <h3>DOCENTES QUE PRESENTARON INFORME FINAL</h3>
        <table>
            <thead>
            <tr>
                <th>N°</th>
        ...
        <h3>DOCENTES QUE NO PRESENTARON EL INFORME FINAL</h3>
        <table>
            <thead><tr>
                <th>N°</th>
                <th>Docente</th>
            ...
            <tbody>
                <?php if (count($guardaron) === 0 && count($no_presentaron) === 0): ?>
                    <tr><td colspan="5">Todos los docentes han enviado su informe final.</td></tr>
                <?php else: ?>
                    <?php $i = 1; ?>
